...
//      where vrcmc.VNF_RESOURCE_CUST_MODEL_CUSTOMIZATION_UUID = 'd279139c-4b85-48ff-8ac4-9b83a6fc6da7') 
   ...
   public List <VfModuleCustomization> getAllVfModuleCustomizations () {
    ...
    /**
     * Return the newest version of a specific Tosca CSAR Record resource (queried by artifactUuid).
     *
     * @param artifactUuid
     * @return ToscaCsar object or null if none found
     */
    public ToscaCsar getToscaCsarByUuid (String artifactUuid) {
        ...
        long startTime = System.currentTimeMillis ();
        LOGGER.debug ("Catalog database - get Tosca CSAR record with artifactUuid: " + artifactUuid);

        String hql = "FROM ToscaCsar WHERE artifactUUID=:artifactUuid";
        Query query = getSession ().createQuery (hql);
        query.setParameter ("artifactUuid", artifactUuid);
